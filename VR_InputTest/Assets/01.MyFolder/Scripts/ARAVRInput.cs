...
//! ARAVRInput 클래스는 VR/AR에 대응하는 Input 기능을 구현하는 클래스다.
public static class ARAVRInput
{
    ...
    public enum Button
    {
        ...
        One = OVRInput.Button.One,                              /**< VR컨트롤러의 One 버튼을 매핑했다. */
        Two = OVRInput.Button.Two,                              /**< VR컨트롤러의 Two 버튼을 매핑했다. */
        Thumbstick = OVRInput.Button.PrimaryThumbstick,         /**< VR컨트롤러의 조이스틱 클릭 버튼을 매핑했다. */
        IndexTrigger = OVRInput.Button.PrimaryIndexTrigger,     /**< VR컨트롤러의 IndexTrigger 버튼을 매핑했다. */
        HandTrigger = OVRInput.Button.PrimaryHandTrigger        /**< VR컨트롤러의 HandTrigger 버튼을 매핑했다. */
#endif      // BUILD_PLATFORM_PC
    }       // Button
...
#if BUILD_PLATFORM_PC
    private static Vector3 originScale = Vector3.one * 0.02f;       /** 크로스 헤어 그릴 때 기존 스케일을 캐싱하는 함수 */
#elif TARGET_DEVICE_OCULUS
    private static Vector3 originScale = Vector3.one * 0.005f;       /** 크로스 헤어 그릴 때 기존 스케일을 캐싱하는 함수 */
#endif      // BUILD_PLATFORM_PC
     ...
     * @return virtualMask에 들어온 값을 ButtonTarget 타입으로 변환해서 리턴한다.
     */
     ...
     * @param crosshair 컨트롤러에 정면에 그릴 크로스헤어 오브젝트의 Transform을 받아온다.
    ...
    public static void DrawCrosshair(Transform crosshair, bool isHand = true, Controller hand = Controller.RTouch)
    {
        ...
        if (plane.Raycast(ray, out distance))
        {
            ...
            // 크로스 헤어의 크기를 최소 기본 크기에서 거리에 따라 더 커지도록 한다.
    ...
    public static void PlayVibration(float duration, float frequency, float amplitude, Controller hand)
    {
        ...
        {
            GameObject coroutineObj = new GameObject("CoroutineInsntace");
            coroutineObj.AddComponent<CoroutineInstance>();
        }       // if : 싱글턴 코루틴 인스턴스의 널 체크를 왜 여기서 함?
     ...
     * @return TrackingSpace의 Transform을 리턴
     */
...
{
    public static CoroutineInstance coroutineInstance = null;   /**< ARAVRInput 기능을 사용하는 동안 메모리에 인스턴스화 할 코루틴 객체 변수이다.*/
